...
class AlgorithmPool:
    def __init__(self, number_population, capacity, lower_threshold, upper_threshold, model_name=None):
        ...
        self.pool = {}#pool = {population_label:[(algorithm, score),(algorithm, score)],[...]}
    ...
    def add_algorithm(self, algorithm, score, population_label, similarity, nmi):
        if population_label not in self.pool:
            self.pool[population_label] = []
        population = self.pool[population_label]#population暂存
        ...
        if similarity > self.upper_threshold:
            ...
            if len(population) >= self.capacity:
                ...
                if population[0][1] < score:#分数更高则替换
                    population[0] = (algorithm, score, nmi)
            else:
                population.append((algorithm, score, nmi))

    def load_algorithm(self, file_path):#从file_path中的pickle文件读取算法
        try:
            with open(file_path, 'rb') as file:
                algorithms_data = pickle.load(file)#反序列化，存储python对象
    ...
    def calculate_embedding(self, code):#code -> matrix转化code为张量
    ...
    def sample_algorithms_population(self, population_label, number):
        ...
        weights = [score for (_, score, _ ) in population]
    ...
    def sample_algorithms(self, number):
        ...
        weights = [score for _, score, _ in all_algorithms]
    ...
    def get_best_algorithm(self):
        ...
        best_nmi = 0
        for population_label, population in self.pool.items():
            for algorithm, score, nmi in population:
                if score > best_score:
                    best_score = score
                    best_algorithm = algorithm
                    best_nmi = nmi

        return (best_algorithm, best_score, best_nmi)
    ...
    def calculate_average_score(self, population_label=None):
        ...
        if population_label:
            ...
            if not population:
                return None
            total_score = sum(score for _, score,_ in population)
            return total_score / len(population)
        else:
            total_score = sum(sum(score for _, score,_ in population) for population in self.pool.values())
    ...
    def calculate_average_nmi(self, population_label=None):
        if not self.pool:
            return None
        ...
        if population_label:
            ...
            if not population:
                return None
            total_nmi = sum(nmi for _,_,nmi  in population)
            return total_nmi / len(population)
        else:
            total_nmi = sum(sum(nmi for _,_, nmi in population) for population in self.pool.values())
            return total_nmi / self.__len__()
